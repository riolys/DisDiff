...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        if args.use_CAE:
            ...
            loss_att = AttM.forward(controller=controller, loss_func=args.loss_func, token2ignore=[4])
        else:
            loss_att = 0
        
        loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean") + loss_att * args.att_param
        ...
        #MSS module
        if args.use_MSS:
            t_norm = custom_function_cos(timesteps, noise_scheduler.config.num_train_timesteps-1).view(-1,1,1,1).cpu()
        ...
        print(f"PGD loss - step {step}, loss: {loss.detach().item():.4f}")
